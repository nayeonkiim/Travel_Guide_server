...
exports.getLocation = async function (result, gender, age) {
    ...
    const findLoc = await Location.findAll({
        ...
        if (gender == 'all' && age == 'all') {
            ...
            for (let t = 0; t < elFilter.length; t++) {
                //해당 관광지에 갔던 user들을 찾아
                const addData = await User.findOne({
                    where: { id: elFilter[t].UserId },
...
exports.getRoutes = async function (necessary, result, gender, age) {
    let route = [];
    let visit = true;
    let member = [];
    let totalMem = [];
    //해당 관광지에 사람들이 간 적이 없는 경우 (아직 데이터 없어)
    if (necessary == null || necessary == undefined || necessary.length == 0)
        visit = false;
    else {
        //console.log(el);
        let curUser = necessary[0].dataValues.UserId;
        let routeNum = [];
        //아이디 별로 routeNum에 넣기
        for (var index in necessary) {
            if (curUser == necessary[index].dataValues.UserId) {
                if (!routeNum.includes(necessary[index].dataValues.TourSubPlaces[0].dataValues.id))
                    routeNum.push(necessary[index].dataValues.TourSubPlaces[0].dataValues.id);
            } else {
                route.push(routeNum);
                routeNum = [];
                curUser = necessary[index].dataValues.UserId;
                routeNum.push(necessary[index].dataValues.TourSubPlaces[0].dataValues.id);
            }
        }
        route.push(routeNum);
    ...
    for (let i = 0; i < member.length; i++) {
        for (let j = 0; j < subPlace.length; j++) {
            //가장 많이 간 경로 순서대로 위도경도 값 넣어주기
            if (member[i] == subPlace[j].id) {
                totalMem.push({ id: subPlace[j].id, name: subPlace[j].name, latitude: subPlace[j].latitude, longitude: subPlace[j].longitude });
            }
        }
    }
